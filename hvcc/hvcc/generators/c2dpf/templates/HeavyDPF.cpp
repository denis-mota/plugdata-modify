...
#include "Heavy_{{name}}.hpp"  // C++ class definition (Heavy_{{name}})
#include "Heavy_{{name}}.h"    // C API (hv_*_new_with_options, hv_*_free)
...
void {{class_name}}::setParameterValue(uint32_t index, float value)
{
  ...
  switch (index) {
    ...
    case {{loop.index-1}}: {
      ...
      {%- elif v.extern == "event" and v.hash %}
      if (value == 1)
        _context->sendBangToReceiver({{v.hash}});
      {%- elif v.hash %}
      _context->sendFloatToReceiver(
        {{v.hash}},
        value);
      {%- else %}
      // do nothing for {{k|upper}} (output parameter)
      {%- endif %}
      break;
    }
    {% endfor %}
